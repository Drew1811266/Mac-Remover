...
# Streaming memory cleanup policy: avoid forcing deep MPS cache sync every chunk.
STREAMING_DEEP_CLEANUP_EVERY = 3
...
def process_single_file(input_path: str, args: argparse.Namespace, device_list: List[str], 
    ...
    if input_type == "video":
        ...
        try:
            print("PROGRESS stage=warmup step=video_backend_ready", flush=True)
        except Exception:
            pass
...
def _stream_video_chunks(
    ...
    while frames_read < frames_to_process:
        ...
        if chunk_idx == 1:
            try:
                print("PROGRESS stage=warmup step=chunk_loop_start", flush=True)
            except Exception:
                pass
        ...
        # Machine-readable progress line for host app runtime parser.
        if total_chunks > 0:
            try:
                print(
                    f"PROGRESS chunk={chunk_idx}/{total_chunks} frames={frames_read}/{frames_to_process}",
                    flush=True,
                )
            except Exception:
                pass
        ...
        # 1) default to non-forced cleanup to avoid frequent MPS synchronization stalls;
        # 2) request deep cleanup every few chunks, still non-forced (pressure-driven).
        if streaming:
            run_deep_cleanup = (chunk_idx % STREAMING_DEEP_CLEANUP_EVERY) == 0
            clear_memory(
                debug=debug,
                deep=run_deep_cleanup,
                force=False,
                timer_name=cleanup_timer_name,
            )
