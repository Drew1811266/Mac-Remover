...
const PYWEBVIEW_READY_TIMEOUT_MS = 10000;
...
async function waitForApi(): Promise<Record<string, (...args: unknown[]) => Promise<unknown>>> {
  // 已就绪时直接复用，避免重复轮询。
  if (hasApiMethods(window.pywebview?.api)) {
  ...
  if (!apiReadyPromise) {
    ...
    apiReadyPromise = new Promise((resolve, reject) => {
      ...
      const interval = window.setInterval(() => {
        // 双保险：轮询 + pywebviewready 事件，兼容不同加载时序。
        if (hasApiMethods(window.pywebview?.api)) {
        ...
        if (Date.now() - started >= PYWEBVIEW_READY_TIMEOUT_MS) {
          window.clearInterval(interval);
          window.removeEventListener('pywebviewready', onReady as EventListener);
          reject(new Error('PyWebView API unavailable (timeout waiting for pywebviewready)'));
        }
      }, 60);
      ...
      window.addEventListener('pywebviewready', onReady as EventListener, { once: true });
    });
  }
